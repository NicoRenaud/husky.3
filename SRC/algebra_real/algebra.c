...
void spec_dsyevr(double *VECT_PRP, double *VAL_PRP, double *MAT, int N) {
	...
	// copie de la matrice dans sa forme vectorielle
	for(i=0;i<N*N;i++)
		VECT_PRP[i] = MAT[i];
	

	// tolerance of the calculation (default = -1)
	abstol = -1.0;
	
	// select what to compute the 10 lowest and highest eigenbalues
	il = (int)N/2-NSELECT/2+1;
	iu = (int)N/2+NSELECT/2;
	z = calloc(N*NSELECT,sizeof(double));

	// define optical worksapce
	lwork = -1;
	liwork = -1;
	dsyevr_( "Vectors", "Indices", "Upper", &n, VECT_PRP, &lda, &vl, &vu, &il, &iu,
                        &abstol, &m, VAL_PRP, z, &ldz, isuppz, &wkopt, &lwork, &iwkopt, &liwork,
                        &info );
	
	// reallocation of the workspace
	lwork = (int)wkopt;
	work = (double *)malloc(lwork*sizeof(double));
	liwork = iwkopt;
	...
	//calcul des vecteurs/valeurs propres;
	dsyevr_( "Vectors", "Indices", "Upper", &n, VECT_PRP, &lda, &vl, &vu, &il, &iu,
                        &abstol, &m, VAL_PRP, z, &ldz, isuppz, work, &lwork, iwork, &liwork,
                        &info );
	
	// check convergence
	if (info>0)
		printf(" === ERROR : DSSYEVR didn't converge\n");
	
	//debug
	printf( "\n The total number of eigenvalues found:%2i\n", m );
	print_vect(VAL_PRP, m, "Vselect");
	
	//printf("Eigenvectors");
	//print_mat(z, N, m, "Uselect");
...
//  Transforme une matrice de la base  locale dans la base propre //
///////////////////////////////////////////////////////////////////
void real_rotateSchmidt(double *M, double *VP, int n)
{
  double *M_temp, *VP_temp;
