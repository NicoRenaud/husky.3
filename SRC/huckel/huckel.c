...
void compute_hamiltonian(double *hmat, double *smat, atom *molecule, int no_atoms, int nb_orb_precomp, char *HKL_PARAM)
{
	...
	int ii,jj,i,j,atom_row,atom_col,noorb_i,noorb_j,indexi,indexj;
	...
	// use the overlap or not
	int use_overlap = 1;
	if(use_overlap==0)
		printf("\t -- Warning: overlap neglected \n");
			...
			for(ii=0;ii<noorb_i;ii++)
			{
				for(jj=0;jj<noorb_j;jj++)
				{
					...
					if(use_overlap==1)
						...
						if( (indexi==indexj) && (ii==jj) )
							smat[ (indexi+ii)*nb_orb_precomp + (indexj+jj) ]= 1.0;
...
int compute_nb_orb(atom *molecule,int nb_atom, int *index_atom, char *HKL_PARAM)
{

	int i, iAt,atom_row,noorb_i;
	int nb_orb = 0;
	read_atomic_parameters(HKL_PARAM);
  ...
  {
			iAt = index_atom[i];
      atom_row=molecule[iAt].atomtype;
...
void create_index_orbitals(int *index_orb_elec_1,int *index_orb_mol,int *index_orb_elec_2,int nb_orb_elec_1,int nb_orb_mol,int nb_orb_elec_2)
{
	...
	// first electrode
	for(i=0;i<nb_orb_elec_1;i++)
	{
		index_orb_elec_1[i] = cpt;
		cpt ++;
	}
	...
	// second electrode
	for(i=0;i<nb_orb_elec_2;i++)
	{
		index_orb_elec_2[i] = cpt;
		cpt ++;
	}
